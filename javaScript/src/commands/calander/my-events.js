...
const { getActiveCalendar, getPublicCalendarForUser, upsertCalendar } = require('../../db');
...
function line(evt) {
  ...
  const eFmt = evt.start?.date
    ? '(all day)'
    : dayjs(e).isValid()
      ? dayjs(e).format('MMM D, HH:mm')
      : String(e);
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(o =>
      o
        .setName('user')
        .setDescription('Another user (must be public)')
        .setRequired(false),
    )
    .addIntegerOption(o =>
      o
    ...
    .addStringOption(o =>
      o
        .setName('provider')
        .setDescription('Calendar provider (google only for now)')
        .addChoices(
  ...
  async execute(interaction) {
    ...
    if (provider !== 'google') {
      return interaction.editReply({ content: 'Only Google is supported right now.', ephemeral: true });
    }

    // Load calendar row
    let row =
      target.id === interaction.user.id
        ? getActiveCalendar(target.id, 'google')
        : getPublicCalendarForUser(target.id, 'google');
    ...
    try {
      ...
      // Build tokens object for Google API
      const tokens = {
      ...
      if (!events.length) {
        return interaction.editReply('No upcoming events.');
      }
      ...
      return interaction.editReply({ content: 'Failed to fetch events.', ephemeral: true });
    }
  },
};
